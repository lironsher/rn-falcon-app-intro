...
class AppIntro extends Component {
				...
				transform: [
					{
						translateX: this.state.parallax.interpolate({
							...
							outputRange: [
								isFirstPage
									? leftPosition
									: leftPosition - offset * level,
								isFirstPage
									? rightPosition + offset * level
									: rightPosition,
							],
						}),
					},
				...
				{this.props.showDoneButton ? (
					<View style={this.styles.viewDoneButton}>
						<DoneButton
							...
							styles={this.styles}
							onNextBtnClick={this.onNextBtnClick.bind(
								this,
								context
							)}
						/>
	...
	render() {
		...
		if (pageArray.length > 0) {
			pages = pageArray.map((page, i) =>
				this.renderBasicSlidePage(i, page)
			);
		} else {
				...
				<Swiper
					...
					onMomentumScrollEnd={(e, state) => {
						if (this.isToTintStatusBar()) {
							StatusBar.setBackgroundColor(
								this.shadeStatusBarColor(
									this.props.pageArray[state.index]
										.backgroundColor,
									-0.3
								),
								false
